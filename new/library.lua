...
do
	...
	function utility:Connection(connectionInfo)
		...
		if connectionInfo.Type then
			...
			--
			rainhook.connections[#rainhook.connections] = connection
			--
			return connection
		end
	end
	...
	function utility:RemoveConnection(connectionInfo)
		...
		if connectionInfo.Connection then
			local found = table.find(rainhook.connections, connectionInfo.Connection)
			--
			if found then
				connectionInfo.Connection:Disconnect()
				--
				table.remove(rainhook.connections, found)
			end
		end
	end
end
...
return library, utility, rainhook