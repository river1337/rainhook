local tools = loadstring(game:HttpGet('https://raw.githubusercontent.com/river1337/rainhook/main/shared/Shared.lua'))()
...
local drawing = {} do
    ...
    local function mouseoverhighersquare(obj)
        for i,square in next, squares do
    ...
    function drawing:new(shape)
        ...
        local mt = setmetatable({exists = true}, {
            __index = function(self, k)
                ...
                elseif k == "AddListLayout" and listfunc then
                    return listfunc
                elseif k == "MakeScrollable" and scrollfunc then
                    return scrollfunc
                elseif k == "RefreshScrolling" and refreshscrolling then
                    return refreshscrolling
                elseif k == "AbsoluteContentSize" then
...
local themes = {
    Default = {
        ...
        ["Dropdown Background"] = Color3.fromRGB(19,19,19)
    },
    Fatality = {
        ["Accent"] = Color3.fromRGB(188,26,108),
        ["Warning"] = Color3.fromRGB(255,165,0),
        ["Window Background"] = Color3.fromRGB(31,26,69),
        ["Window Border"] = Color3.fromRGB(38,33,50),
        ["Tab Background"] = Color3.fromRGB(26,19,51),
        ["Tab Border"] = Color3.fromRGB(47,39,76),
        ["Tab Toggle Background"] = Color3.fromRGB(47,39,76),
        ["Section Background"] = Color3.fromRGB(31,26,69),
        ["Section Border"] = Color3.fromRGB(47,39,76),
        ["Text"] = Color3.fromRGB(200,200,200),
        ["Disabled Text"] = Color3.fromRGB(125,123,162),
        ["Object Background"] = Color3.fromRGB(27,20,63),
        ["Object Border"] = Color3.fromRGB(51,44,88),
        ["Dropdown Background"] = Color3.fromRGB(188,26,108)
    },
}
local themeobjects = {}
...
local decode = function(data)
    return tools:base64_decode(data)
end
...
local flags = {}
local configignores = {}
function library:SaveConfig(Name, GameName)
    if type(Name) == "string" and Name:find("%S+") and Name:len() > 1 then
        ...
        local configtbl = {}
        local placeid = GameName and "universal" or game.PlaceId
...
function library:DeleteConfig(Name, GameName)
    assert(self.folder, "No folder specified")
    assert(self.extension, "No file extension specified")
    local placeid = GameName and "universal" or game.PlaceId
    local folderpath = string.format("%s//%s", self.folder, placeid)
...
function library:LoadConfig(Name, GameName)
...
function library:GetConfigs(GameName)
    assert(self.folder, "No folder specified")
    assert(self.extension, "No file extension specified")
    local configs = {}
    ...
    if GameName and isfolder(universalfolder) then
        for i,config in next, (isfolder(placeidfolder) and listfiles(placeidfolder) or {}) do
...
function library:GetThemes()
    local themes = {"Default","Fatality"}
    local folderpath = string.format("%s//themes", self.folder)
    if isfolder(folderpath) then
        for i,theme in next, listfiles(folderpath) do
...
function library:CreateDropDown(holder, content, flag, callback, default, max, scrollable, scrollingmax, islist, section, sectioncontent)
    ...
    function dropdowntypes:Refresh(tbl)
        ...
        count = 0
        for i,opt in next, optioninstances do
            coroutine.wrap(function()
                opt.button:Remove()
            end)()
        end
...
local pickers = {}
function library:CreateColourPicker(default, defaultalpha, parent, count, flag, callback)
    ...
    local window = utility.create("Square", {
        ...
        Visible = false,
        Position = UDim2.new(1, -176 + (count * 18) + (count * 6), 1, 6),
        ZIndex = 11
    })
    ...
    local function set(color, a, nopos)
        ...
        if hsv ~= oldcolor or alpha ~= oldalpha then
            ...
            if flag then 
                library.flags[flag] = utility.rgb(hsv.r * 255, hsv.g * 255, hsv.b * 255)
            end
            callback(utility.rgb(hsv.r * 255, hsv.g * 255, hsv.b * 255))
        end
    end
    flags[flag] = set
    set(default, defaultalpha)
    local defhue, i,_ = default:ToHSV()
    local curhuesizey = defhue
    library:CreateTextBox(rgbinput, text, function(str) 
        if str == "" then
            text.Visible = false
    ...
    icon.MouseButton1Click:Connect(function()
        for i,picker in next, pickers do
...
function library:Watermark(Message)
    local size = utility.textlength(Message, Drawing.Fonts.Plex, 13).X
    ...
    local open = true
    function watermarktypes:Visible(Enabled)
        watermark.Visible = Enabled
    end
    function watermarktypes:Position(X,Y)
...
function library:Load(options)
    ...
    self.holder = holder
    local TextName = utility.create("Text", {
        Text = name,
        Font = Drawing.Fonts.Plex,
        Size = 13,
    ...
    function windowtypes:AddTab(name)
        ...
        local column2 = utility.create("Square", {
            ...
            Position = UDim2.new(0.332, 4, 0, 0)
        })
        column2:AddListLayout(12)
        column2:MakeScrollable()
        local column3 = utility.create("Square", {
            Transparency = 0,
            Parent = tab,
            Size = UDim2.new(0.34, -4, 1, 0),
            Position = UDim2.new(0.66, 4, 0, 0)
        })
        column3:AddListLayout(12)
        column3:MakeScrollable()
        local mouseover = false
        ...
        function tabtypes:AddSection(options)
            utility.table(options)
            local name = options.name
            local side = options.side and options.side:lower()
            local column = side == "left" and column1 or side == "middle" and column2 or side == "right" and column3
            ...
            function sectiontypes:AddLabel(name)
            ...
            function sectiontypes:AddButton(options)
                utility.table(options)
                local name = options.name
            ...
            function sectiontypes:AddToggle(options)
                utility.table(options)
                local name = options.name
                ...
                function toggletypes:AddToggle(bool)
                    set(bool)
                end
                local colorpickers = -1
                function toggletypes:AddColourPicker(options)
                    ...
                    return library:CreateColourPicker(default, defaultalpha, holder, colorpickers, flag, callback)
                end
                function toggletypes:AddKeybind(options)
                    utility.table(options)
                ...
                function toggletypes:AddSlider(options)
                    utility.table(options)
                ...
                function toggletypes:AddDropdown(options)
                    utility.table(options)
                    local default = options.default
            ...
            function sectiontypes:AddTextBox(options)
                utility.table(options)
                ...
                library:CreateTextBox(box, text, function(str) 
                    if str == "" then
                        text.Visible = false
                        placeholdertext.Visible = true
                    else
            ...
            function sectiontypes:AddSlider(options)
                utility.table(options)
                local name = options.name
            ...
            function sectiontypes:AddDropdown(options)
                utility.table(options)
                local name = options.name
                local default = options.default
                local content = type(options.content) == "table" and options.content or {}
            ...
            function sectiontypes:AddColourPicker(options)
                utility.table(options)
                local name = options.name
                local default = options.default or Color3.fromRGB(255, 255, 255)
                local flag = options.flag or utility.nextflag()
                ...
                local colorpickertypes = library:CreateColourPicker(default, defaultalpha, holder, colorpickers, flag, callback)
                function colorpickertypes:AddColourPicker(options)
                    colorpickers = colorpickers + 1
                    utility.table(options)
                    local default = options.default or Color3.fromRGB(255, 255, 255)
                    local flag = options.flag or utility.nextflag()
            ...
            function sectiontypes:AddKeybind(options)
                utility.table(options)
                local name = options.name
                local default = options.default
                local blacklist = options.blacklist or {}
