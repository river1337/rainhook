--[[
	...
	[i] This version is degraded to support free / weaker exploits
...
local gameMetatable = getrawmetatable(game) -- "getrawmetatable" is just an empty / unsecure function for some free exploits.
local __index = gameMetatable.__index

]]

local __index_aux = function(Object, Key) -- Auxillary function, alternative to __index but slower since "getrawmetatable" isn't supported.
	return Object[Key]
end

local GetService = __index_aux(game, "GetService")
local FindFirstChild, WaitForChild = __index_aux(game, "FindFirstChild"), __index_aux(game, "WaitForChild")
local IsA = __index_aux(game, "IsA")
...
local CurrentCamera = __index_aux(Workspace, "CurrentCamera")
local LocalPlayer = __index_aux(Players, "LocalPlayer")
...
local Cache = {
	WorldToViewportPoint = __index_aux(CurrentCamera, "WorldToViewportPoint"),
	GetPlayers = __index_aux(Players, "GetPlayers"),
	GetPlayerFromCharacter = __index_aux(Players, "GetPlayerFromCharacter"),
	GetMouseLocation = __index_aux(UserInputService, "GetMouseLocation")
}
...
local IsDescendantOf = function(self, ...)
	return typeof(self) == "Instance" and __index_aux(self, "IsDescendantOf")(self, ...)
end

--[[

local Connect, Disconnect, GetRenderProperty, SetRenderProperty = __index_aux(game, "DescendantAdded").Connect

do
	local TemporaryDrawing = Drawingnew("Line")
	GetRenderProperty = getupvalue(getmetatable(TemporaryDrawing).__index_aux, 4)
	SetRenderProperty = getupvalue(getmetatable(TemporaryDrawing).__newindex, 4)
	TemporaryDrawing.Remove(TemporaryDrawing)

	local TemporaryConnection = Connect(__index_aux(game, "DescendantAdded"), function() end)
...
]] -- Too advanced for free exploits.
...
local Connect, Disconnect = function(self, Closure)
	return self:Connect(Closure)
end, function(self)
...
local CoreFunctions = {
	...
	GetLocalCharacterPosition = function()
		local LocalCharacter = __index_aux(LocalPlayer, "Character")
		local LocalPlayerCheckPart = LocalCharacter and (__index_aux(LocalCharacter, "PrimaryPart") or FindFirstChild(LocalCharacter, "Head"))

		return LocalPlayerCheckPart and __index_aux(LocalPlayerCheckPart, "Position") or __index_aux(CurrentCamera, "CFrame").Position
	end,
	...
	CalculateParameters = function(Object)
		...
		local Part = IsPlayer and (FindFirstChild(Players, __index_aux(Object, "Name")) and __index_aux(Object, "Character"))
		Part = IsPlayer and (Part and (__index_aux(Part, "PrimaryPart") or FindFirstChild(Part, "HumanoidRootPart"))) or Object

		if not Part or IsA(Part, "Player") then
			return nil, nil, false
		end

		local PartCFrame, PartPosition, PartUpVector = __index_aux(Part, "CFrame"), __index_aux(Part, "Position")
		PartUpVector = PartCFrame.UpVector

		local CameraUpVector = __index_aux(CurrentCamera, "CFrame").UpVector
	...
	GetColor = function(Player, DefaultColor)
		...
		return Settings.EnableTeamColors and __index_aux(Player, TeamCheckOption) == __index_aux(LocalPlayer, TeamCheckOption) and Settings.TeamColor or DefaultColor
	end
}
...
local UpdatingFunctions = {
	ESP = function(Entry, TopTextObject, BottomTextObject)
		...
		if GetRenderProperty(TopTextObject, "Visible") then
			...
			local Character = IsAPlayer and __index_aux(Player, "Character") or Player
			local Humanoid = FindFirstChildOfClass(Character, "Humanoid")
			local Health, MaxHealth = Humanoid and __index_aux(Humanoid, "Health") or Nan, Humanoid and __index_aux(Humanoid, "MaxHealth") or Nan
			...
			local PlayerPosition = __index_aux((IsAPlayer and (__index_aux(Character, "PrimaryPart") or __index_aux(Character, "Head")) or Character), "Position") or Vector3zero
	...
	Tracer = function(Entry, TracerObject, TracerOutlineObject)
		...
		if GetRenderProperty(TracerObject, "Visible") then
			...
			local CameraViewportSize = __index_aux(CurrentCamera, "ViewportSize")
	...
	HeadDot = function(Entry, CircleObject, CircleOutlineObject)
		...
		local Character = Entry.IsAPlayer and __index_aux(Entry.Object, "Character") or __index_aux(Entry.Object, "Parent")
		...
		local HeadCFrame, HeadSize = __index_aux(Head, "CFrame"), __index_aux(Head, "Size")
	...
	HealthBar = function(Entry, MainObject, OutlineObject, Humanoid)
		...
		if GetRenderProperty(MainObject, "Visible") then
			...
			Humanoid = Humanoid or FindFirstChildOfClass(__index_aux(Entry.Object, "Character"), "Humanoid")

			local MaxHealth = Humanoid and __index_aux(Humanoid, "MaxHealth") or 100
			local Health = Humanoid and mathclamp(__index_aux(Humanoid, "Health"), 0, MaxHealth) or 0
	...
	Chams = function(Entry, Part, Cham)
		...
		local _CFrame, PartSize = select(2, pcall(function()
			return __index_aux(Part, "CFrame"), __index_aux(Part, "Size") / 2
		end))
...
local CreatingFunctions = {
	ESP = function(Entry)
		...
		Entry.Connections.ESP = Connect(__index_aux(RunService, Environment.DeveloperSettings.UpdateMode), function()
	...
	Tracer = function(Entry)
		...
		SetRenderProperty(TracerObject, "ZIndex", -2)
		...
		Entry.Connections.Tracer = Connect(__index_aux(RunService, Environment.DeveloperSettings.UpdateMode), function()
			local Functionable, Ready = pcall(function()
				return Environment.Settings.Enabled and Settings.Enabled and Entry.Checks.Ready
			end)
	...
	HeadDot = function(Entry)
		...
		if not Entry.IsAPlayer and not Entry.PartHasCharacter then
			if not FindFirstChild(__index_aux(Entry.Object, "Parent"), "Head") then
				return
			end
		end
		...
		Entry.Connections.HeadDot = Connect(__index_aux(RunService, Environment.DeveloperSettings.UpdateMode), function()
			local Functionable, Ready = pcall(function()
				return Environment.Settings.Enabled and Settings.Enabled and Entry.Checks.Ready
			end)
	...
	Box = function(Entry)
		...
		Entry.Connections.Box = Connect(__index_aux(RunService, Environment.DeveloperSettings.UpdateMode), function()
			local Functionable, Ready = pcall(function()
				return Environment.Settings.Enabled and Settings.Enabled and Entry.Checks.Ready
			end)
	...
	HealthBar = function(Entry)
		...
		local Humanoid = FindFirstChildOfClass(__index_aux(Entry.Object, "Parent"), "Humanoid")
		...
		Entry.Connections.HealthBar = Connect(__index_aux(RunService, Environment.DeveloperSettings.UpdateMode), function()
			local Functionable, Ready = pcall(function()
				return Environment.Settings.Enabled and Settings.Enabled and Entry.Checks.Ready
			end)
	...
	Chams = function(Entry)
		...
		elseif not Entry.IsAPlayer then
			xpcall(function()
				ChamsEntry[__index_aux(Object, "Name")] = {}
			end, function()
				Cancel = true
			end)
		end
		...
		Entry.Connections.Chams = Connect(__index_aux(RunService, Environment.DeveloperSettings.UpdateMode), function()
			for Index, Value in next, ChamsEntry do
				local Character = Entry.IsAPlayer and __index_aux(Object, "Character") or __index_aux(Object, "Parent")
	...
	Crosshair = function()
		...
		ServiceConnections.UpdateCrosshairProperties, ServiceConnections.UpdateCrosshair = Connect(__index_aux(RunService, DeveloperSettings.UpdateMode), function()
			if Settings.Enabled and Environment.Settings.Enabled then
				if Settings.Position == 1 then
					Axis = GetMouseLocation()
				elseif Settings.Position == 2 then
					Axis = __index_aux(CurrentCamera, "ViewportSize") / 2
				else
					Settings.Position = 1
				end
		...
		end), Connect(__index_aux(RunService, DeveloperSettings.UpdateMode), function()
...
local UtilityFunctions = {
	InitChecks = function(self, Entry)
		...
		Entry.Connections.UpdateChecks = Connect(__index_aux(RunService, DeveloperSettings.UpdateMode), function()
			local RenderDistance = Entry.RenderDistance

			if not Entry.IsAPlayer and not Entry.PartHasCharacter then
				Checks.Ready = (__index_aux(Player, "Position") - CoreFunctions.GetLocalCharacterPosition()).Magnitude <= RenderDistance; return
			end

			local PartHumanoid = FindFirstChildOfClass(__index_aux(Player, "Parent"), "Humanoid")
			...
			if not IsAPlayer then
				...
				if Settings.AliveCheck then
					Checks.Alive = __index_aux(PartHumanoid, "Health") > 0
				end
			...
			local Character = __index_aux(Player, "Character")
			...
			if Character and IsDescendantOf(Character, Workspace) and Humanoid and Head then
				...
				if Settings.AliveCheck then
					Checks.Alive = __index_aux(Humanoid, "Health") > 0
				end

				if Settings.TeamCheck then
					Checks.Team = __index_aux(Player, TeamCheckOption) ~= __index_aux(LocalPlayer, TeamCheckOption)
				end
			else
				Checks.Alive = false
				Checks.Team = false
			...
			local IsInDistance = select(2, pcall(function()
				return (IsAPlayer and __index_aux(Head, "Position") or __index_aux(Player, "Position") - CoreFunctions.GetLocalCharacterPosition()).Magnitude <= RenderDistance or true
			end))
			...
			if Checks.Ready then
				Entry.RigType = Humanoid and (__index_aux(Humanoid, "RigType") == 0 and "R6" or "R15") or "N/A"
			end
		end)
	end,
	...
	WrapObject = function(self, Object, PseudoName, Allowed, RenderDistance)
		...
		if pcall(gethiddenproperty, Object, "PrimaryPart") then
			Object = __index_aux(Object, "PrimaryPart")
		end
		...
		local Entry = {
			...
			Object = Object,
			Allowed = Allowed,
			Name = PseudoName or __index_aux(Object, "Name"),
			DisplayName = PseudoName or __index_aux(Object, (IsA(Object, "Player") and "Display" or "").."Name"),
		...
		repeat wait(0) until Entry.IsAPlayer and FindFirstChildOfClass(__index_aux(Entry.Object, "Character"), "Humanoid") or true

		if not Entry.IsAPlayer then
			if not pcall(function()
				return __index_aux(Entry.Object, "Position"), __index_aux(Entry.Object, "CFrame")
			end) then
				warn("EXUNYS_ESP > UtilityFunctions.WrapObject - Attempted to wrap object of an unsupported class type: \""..(__index_aux(Entry.Object, "ClassName") or "N / A").."\"")
				return self.UnwrapObject(Entry.Hash)
			end

			Entry.Connections.UnwrapSignal = Connect(Entry.Object.Changed, function(Property)
				if Property == "Parent" and not IsDescendantOf(__index_aux(Entry.Object, Property), Workspace) then
					self.UnwrapObject(nil, Entry.Hash)
				end
			end)
		end

		local Humanoid = Entry.IsAPlayer and FindFirstChildOfClass(__index_aux(Entry.Object, "Character"), "Humanoid") or FindFirstChildOfClass(__index_aux(Entry.Object, "Parent"), "Humanoid")
		...
		Entry.Connections.PlayerUnwrapSignal = Connect(Entry.Object.Changed, function(Property)
			if DeveloperSettings.UnwrapOnCharacterAbsence and Property == "Parent" and not IsDescendantOf(__index_aux(Entry.Object, (Entry.IsAPlayer and "Character" or Property)), Workspace) then
				self.UnwrapObject(nil, Entry.Hash)
			end
		end)
...
local LoadESP = function()
	...
	ServiceConnections.PlayerRemoving = Connect(__index_aux(Players, "PlayerRemoving"), UtilityFunctions.UnwrapObject)

	ServiceConnections.CharacterAdded = Connect(__index_aux(Workspace, "DescendantAdded"), function(Object)
		if not IsA(Object, "Model") then
			return
		end

		if not GetPlayerFromCharacter(Object) or not FindFirstChild(Players, __index_aux(Object, "Name")) then
			return
		end
		...
		for _, Value in next, GetPlayers() do
			...
			for _, _Value in next, Environment.UtilityAssets.WrappedObjects do
				...
				if __index_aux(_Value.Object, "Name") == __index_aux(Value, "Name") then
					Player = _Value
				end
			end
	...
	ServiceConnections.PlayerAdded = Connect(__index_aux(Players, "PlayerAdded"), function(Player)
		local WrappedObjects = Environment.UtilityAssets.WrappedObjects
		...
		for _, Entry in next, WrappedObjects do
			...
			Entry.Connections[__index_aux(Player, "Name").."CharacterAdded"] = Connect(__index_aux(Player, "CharacterAdded"), function(Object)
				for _, _Value in next, Environment.UtilityAssets.WrappedObjects do
					if not _Value.Name == __index_aux(Object, "Name") then
						continue
					end
